      ...
      <div className={classes.MainContainer}>
        <div className={classes.PlayerSection}>
            ...
            {this.state.showLoader ? null : (
              <div>
                <iframe
                  ...
                  title={"Video"}
                  src={`https://player.vimeo.com/video/${this.state.videoData.vimeoId}?title=0&byline=0`}
                  frameborder="0"
          ...
          <div className={classes.PlaylistSection}>
            <div className={classes.Playlist}>{videoCards}</div>
          </div>
        </div>
      </div>
    );
...
const mapDispatchToProps = dispatch => {
  return {
    Increments: id => dispatch({ type: "INC", val: id })
  };
};

export default connect(null, mapDispatchToProps)(WatchPage);
